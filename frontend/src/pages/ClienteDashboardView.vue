<template>
  <section class="section client-agenda-section">
    <div class="container">
      <div class="header-actions">
        ...
        <button class="btn btn-outline" @click="carregarAgendamentos">🔄 Atualizar</button>
      </div>
        ...
        <div v-for="agendamento in agendamentos" :key="agendamento.id" class="card agendamento-card">
          ...
          <div class="card-footer actions">
            <button 
              ...
              :disabled="agendamento.status === 'CANCELADO' || agendamento.status === 'CONCLUIDO'"
            >
              Remarcar
            </button>
            <button 
      ...
      <div v-if="agendamentoSelecionado" class="modal-overlay" @click.self="fecharRemarcar">
        <div class="card remarcar-card">
          ...
          <p class="modal-subtitle">Serviço: <strong>{{ agendamentoSelecionado.servico.nome }}</strong></p>

          <div class="field">
            <label>Selecione a nova data</label>
            <input type="date" v-model="novaData" class="input custom-input" />
          </div>
...
const carregarAgendamentos = async () => {
  ...
  try {
    ...
    Swal.fire({
      ...
      icon: 'error',
      title: 'Ops!',
      text: 'Erro ao carregar seus agendamentos.'
    })
  } finally {
    loading.value = false
  }
...
const cancelarAgendamento = async (id) => {
  const confirmacao = await Swal.fire({
    ...
    text: "Esta ação cancelará o seu horário reservado.",
    icon: 'warning',
  ...
  if (confirmacao.isConfirmed) {
    try {
      ...
      Swal.fire({
        ...
        icon: 'success',
        title: 'Cancelado',
        text: 'O seu agendamento foi cancelado com sucesso.'
      })
    } catch (error) {
      console.error(error)
      Swal.fire({
...
const remarcar = async (horario) => {
  try {
    ...
    Swal.fire({
      ...
      icon: 'success',
      title: 'Remarcado!',
      text: 'O seu horário foi alterado com sucesso.'
    })
    ...
    Swal.fire({
      ...
      icon: 'error',
      title: 'Erro',
      text: 'Não foi possível remarcar o seu agendamento.'
    })
  }
}
...
.status-green { background: rgba(34, 197, 94, 0.15); color: #4ade80; border: 1px solid rgba(34, 197, 94, 0.2); }
.status-red { background: rgba(239, 68, 68, 0.15); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.2); }
.status-yellow { background: rgba(250, 204, 21, 0.15); color: #fde047; border: 1px solid rgba(250, 204, 21, 0.2); }
.status-gray { background: rgba(156, 163, 175, 0.15); color: #9ca3af; border: 1px solid rgba(156, 163, 175, 0.2); }

/* ==========================================
   MODAL DE REMARCAÇÃO E INPUTS ESCUROS
   ========================================== */
.modal-overlay {
  position: fixed;
  inset: 0;
...
/* Força o calendário do input a ficar escuro */
.custom-input {
